...
make || echo "cc65 build failed, continuing..."
sudo make install || echo "cc65 install failed, continuing..."
...
cd wla-dx-10.2
make || echo "WLA-DX build failed, continuing..."
sudo make install || echo "WLA-DX install failed, continuing..."
...
if ! grep -q "DEVKITPRO" ~/.bashrc; then
  ...
  echo 'export PATH=$DEVKITPRO/tools/bin:$DEVKITARM/bin:$DEVKITA64/bin:$PATH' >> ~/.bashrc
fi
source ~/.bashrc
